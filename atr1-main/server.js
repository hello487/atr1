...
const MCSMClient = require('./mcsm-api.js');
...
// 停止MCSM实例
app.post('/api/mcsm/instances/:instanceId/stop', requireAdminAuth, async (req, res) => {
...
// 重启MCSM实例
app.post('/api/mcsm/instances/:instanceId/restart', requireAdminAuth, async (req, res) => {
  const { instanceId } = req.params;
...
// 发送命令到MCSM实例
app.post('/api/mcsm/instances/:instanceId/command', requireAdminAuth, async (req, res) => {
  const { instanceId } = req.params;
...
// 获取MCSM实例控制台日志
app.get('/api/mcsm/instances/:instanceId/log', requireAdminAuth, async (req, res) => {
  const { instanceId } = req.params;
...
// 更改MCSM实例端口
app.put('/api/mcsm/instances/:instanceId/port', requireAdminAuth, async (req, res) => {
  const { instanceId } = req.params;
  const { port } = req.body;
...
// 获取所有MCSM用户
app.get('/api/mcsm/users', requireAdminAuth, async (req, res) => {
  try {
...
// 创建MCSM用户
app.post('/api/mcsm/users', requireAdminAuth, async (req, res) => {
...
// 删除MCSM用户
app.delete('/api/mcsm/users/:userId', requireAdminAuth, async (req, res) => {
...
// 绑定用户到实例
app.post('/api/mcsm/users/:userId/instances/:instanceId/bind', requireAdminAuth, async (req, res) => {
...
// 解绑用户从实例
app.delete('/api/mcsm/users/:userId/instances/:instanceId/bind', requireAdminAuth, async (req, res) => {
  const { userId, instanceId } = req.params;
...
// 获取用户绑定的实例列表
app.get('/api/mcsm/users/:userId/instances', requireAdminAuth, async (req, res) => {
  const { userId } = req.params;
...
// 获取实例绑定的用户列表
app.get('/api/mcsm/instances/:instanceId/users', requireAdminAuth, async (req, res) => {
  const { instanceId } = req.params;
...
// 用户获取自己的实例列表
app.get('/api/user/instances', requireUserAuth, async (req, res) => {
  const userId = req.user.id;
...
// 用户发送命令到自己的实例
app.post('/api/user/instances/:instanceId/command', requireUserAuth, async (req, res) => {
  const { instanceId } = req.params;
  const { command } = req.body;
  const userId = req.user.id;
...
// 用户获取自己实例的控制台日志
app.get('/api/user/instances/:instanceId/log', requireUserAuth, async (req, res) => {
  const { instanceId } = req.params;
  const { lines = 50 } = req.query;
  const userId = req.user.id;
